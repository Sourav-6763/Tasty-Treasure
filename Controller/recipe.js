...
const searchRecipeById = async (req, res, next) => {
  ...
  try {
    ...
    const similarRecipeResponse = await axios.get(`https://api.spoonacular.com/recipes/${id}/similar`, {
      params: {
        apiKey: process.env.SPOONACULAR_API_KEY,  // The API key for authorization
      },
    });
...
const quickAns=async(req,res,next)=>{
  ...
  try {
   const response= await axios.get(`https://api.spoonacular.com/recipes/quickAnswer`,{
   params:{
  apiKey: process.env.SPOONACULAR_API_KEY,
  q:question
    }
